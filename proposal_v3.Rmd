...
Yet in spite of these advances, machine learning for natural language processing (NLP) remains a largely ad-hoc field. Save a handful of empirical laws, there is little statistical theory guiding the modeling of textual data. What theory does exist generally does not inform specification or use of statistical or machine learning models of text. Instead, the field has relied on icreasingly complex models, requiring tremendous computational power, to drive these advances. Is it possible that statistical theory cold

Figure 1 depicts the number of parameters in the currently most-popular class of machine learning models of text, known as "transformers". Transformers are named after the core neural network architecture that drives them. (cite "attention is all you need" paper) These deep neural network models are not only complex, they are expensive to train. GPT-3 (not pictured), the latest and greatest member of this class has approxmiately 17.5 billion parameters and cost an estimated $12 million to train. (cite) 
